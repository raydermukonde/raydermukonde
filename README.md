<!-- Banner / Introduction -->
<h1 align="center">👋 Hi, I'm <span style="color:#38BDF8;">Marc Mukonde</span></h1>
<h3 align="center">💡 Founder of GreenPulse & Authen-Tech | Flutter & Next.js Developer | Building Africa-focused Tech</h3>

<p align="center">
  <a href="https://linkedin.com/in/tonprofil"><img src="https://img.shields.io/badge/LinkedIn-0077B5?style=flat&logo=linkedin&logoColor=white"/></a>
  <a href="mailto:marc.mukonde@email.com"><img src="https://img.shields.io/badge/Email-Contact%20Me-ff6b6b?style=flat&logo=gmail&logoColor=white"/></a>
  <img src="https://img.shields.io/badge/From-DRCongo-32CD32?style=flat&logo=earth"/>
</p>

---

## 🌍 About Me

I'm a **self-taught developer and entrepreneur** from the **Democratic Republic of Congo (DRC)**.  
I build modern, scalable solutions addressing real African challenges — from **digital payments** to **document verification** and **data security**.

> 🧭 *"Technology isn’t about replacing people — it’s about empowering them."*  
> — **Marc Mukonde**

---

## 🧠 Tech Stack

<p align="center">
  <img src="https://img.shields.io/badge/Flutter-02569B?style=for-the-badge&logo=flutter&logoColor=white"/>
  <img src="https://img.shields.io/badge/Next.js-000000?style=for-the-badge&logo=nextdotjs&logoColor=white"/>
  <img src="https://img.shields.io/badge/Firebase-FFCA28?style=for-the-badge&logo=firebase&logoColor=black"/>
  <img src="https://img.shields.io/badge/Supabase-3ECF8E?style=for-the-badge&logo=supabase&logoColor=black"/>
  <img src="https://img.shields.io/badge/Google_ML_Kit-4285F4?style=for-the-badge&logo=google&logoColor=white"/>
  <img src="https://img.shields.io/badge/OpenCV-5C3EE8?style=for-the-badge&logo=opencv&logoColor=white"/>
</p>

---

## 🚀 Featured Projects

### 🛡️ [Authen-Tech](https://github.com/marc-mukonde/authen-tech)
> Flutter app for **document authenticity verification**

- 🔍 Text extraction with ML Kit  
- ⚖️ Text comparison using DiffMatchPatch  
- 🧠 Visual analysis with OpenCV  
- ☁️ Data storage via Firebase  

---

### 💸 [GreenPulse](https://github.com/marc-mukonde/greenpulse)
> Contactless payment platform for **African SMEs**

- 📱 QR code–based payment system  
- 💳 Integration with Airtel Money  
- 📊 Merchant dashboard with analytics  

---

### 🔒 SafeBox
> Secure digital vault with AES encryption and biometric access  

- 🧩 Flutter + Firebase integration  
- 🔐 Auto-lock & PIN fallback  
- 🪶 Minimalist user interface  

---

## 🧩 Currently Learning

- 🤖 LangChain & conversational AI  
- 🌐 Payment API integrations (Flutterwave, Paystack, Airtel Money)  
- ⚡ Smart home & IoT systems (Home Assistant, Solar monitoring)

---

## 📊 GitHub Stats

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=marc-mukonde&show_icons=true&theme=tokyonight" height="160px"/>
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=marc-mukonde&layout=compact&theme=tokyonight" height="160px"/>
</p>

---

## 📫 Contact

<p align="center">
  📧 <a href="mailto:marc.mukonde@email.com">marc.mukonde@email.com</a>  
  <br/>
  🔗 <a href="https://linkedin.com/in/tonprofil">LinkedIn</a> | <a href="https://tonsite.com">Portfolio</a>
</p>

---

# 🇫🇷 En résumé

👋 Salut, je suis **Marc Mukonde**, développeur et entrepreneur congolais passionné par l’innovation technologique.  
Je conçois des solutions **FinTech** et **d’authentification documentaire** adaptées aux réalités africaines.

### 🚀 Mes projets
- **GreenPulse** : solution de paiement par QR code  
- **Authen-Tech** : vérification d’authenticité de documents  
- **SafeBox** : coffre-fort numérique sécurisé  

🎯 Mon objectif : bâtir des solutions africaines à impact mondial.  

---

<p align="center">
  💬 *“If you can dream it, you can build it.”*  
</p>

📧 marc.mukonde@email.com  
🔗 [LinkedIn](https://linkedin.com/in/...) | [Portfolio](https://...)  
